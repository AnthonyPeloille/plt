...
render::ElementTabLayer::ElementTabLayer(const state::ElementTab &tab, const state::State &state, int gridTileSet):tab(tab) {
    if (gridTileSet == 1) {
        ...
        for(size_t j = 0; j < this->tab.getWidth(); j++) {
            ...
            if(this->tab.get(i,j) != NULL) {
                ...
                if (tid == state::WALL) {
                    ...
                    if(e->getColored()){
                        this->surface->setSprite(this->tileset->getTile(std::stoi(tileId.str())), i, j, sf::Color::Green);
                    }else{
                        this->surface->setSprite(this->tileset->getTile(std::stoi(tileId.str())), i, j);
                    }
